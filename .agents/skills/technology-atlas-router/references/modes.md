...
| `verify` | 完成・署名・Digestを確認する | Release URI、Digest、署名鍵、Certificate、検証状態、Completion class |
...
どのModeでも、`app/data/index.generated.json`の`digest`を返却の根拠にし、固定ReleaseがあればCompletion classを省略しない。候補が複数ならScopeとExclusionで絞り、推測で1件に固定しない。
