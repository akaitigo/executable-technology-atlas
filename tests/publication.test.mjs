...
test('Subject完成は公開Trustと検証済みCertificateを必須にする',async()=>{const index=JSON.parse(await readFile(path.join(root,'app/data/index.generated.json'),'utf8'));const completed=index.subjects.filter((item)=>item.release?.status==='complete'&&item.release?.certificateVerification.status==='verified'&&item.release?.trust.usage==='public-release');assert.equal(completed.length,0);});
