...
test('lockfile取込、非後退、Graph負例、Gate、Core audit、Certificate、root Definitive実失敗、非変更を全て検証する',()=>{for(const value of ['npm ci --ignore-scripts --no-audit --no-fund','npm run non-regression','npm test','npm run dependency:graph:check','npm run dependency:negative','--gate evidence-dependency','影響Evidenceがstaleのままです: output.public.data.index.76090e9b725c469db2cfbe02a8c5f3ab8874b6422cba88d37bd7bec6e7aed3b0.json','npm run definitive:root:check','npm run gate','atlas audit','certificate verify','git diff --exit-code'])assert.ok(source.includes(value));});
test('署名境界以降を公開allowed signersでgit verify-commitする',()=>{for(const value of ['e4e3e48fbc270789505c90c1433f8ccc6a2900fb^..HEAD','gpg.format=ssh','security/allowed_signers','verify-commit'])assert.ok(source.includes(value));assert.doesNotMatch(source,/force-push|reset --hard|rebase/);});
