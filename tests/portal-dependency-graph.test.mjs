...
test('全Evidence wrapper・Core report・公開Indexをrequired outputから退避できない',async()=>{const graph=JSON.parse(await readFile(path.join(root,'evidence/dependency-graph.json'),'utf8'));const discovered=await discoverPortalOutputs(root);const bootstrap=JSON.parse(await readFile(path.join(root,'app/data/index-bootstrap.generated.json'),'utf8'));assert.deepEqual(graph.required_outputs,discovered);for(const required of ['app/data/index-bootstrap.generated.json',`public${bootstrap.publicUrl}`,'evidence/import.integrity.evidence.json','evidence/non-regression.evidence.json','evidence/reports/performance.json','evidence/reports/router-eval.json','provenance.yaml'])assert.ok(graph.required_outputs.includes(required),required);});
