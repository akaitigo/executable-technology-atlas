...
test('Evidence wrapperとGraph freshnessを実再実行entrypointへ結ぶ',async()=>{
  ...
  assert.ok(['blocked','ready'].includes(report.status));assert.equal(report.summary.wrappers,8);assert.equal(report.summary.currentWrappers+report.summary.staleWrappers,8);assert.equal(report.summary.inputs,4);assert.equal(report.summary.recordedOutputs+report.summary.missingDiscoveredOutputs,report.summary.discoveredOutputs);assert.equal(report.summary.discoveredOutputs,39);assert.equal(report.boundary.completionEffect,'none');if(report.status==='ready'){assert.equal(report.summary.currentWrappers,8);assert.equal(report.summary.staleWrappers,0);assert.equal(report.summary.inputsChangedSinceRun,0);assert.equal(report.summary.missingDiscoveredOutputs,0);assert.equal(report.summary.graphCurrent,true);assert.equal(report.summary.currentRerun,true);}else{assert.equal(report.summary.currentRerun,false);}
});
