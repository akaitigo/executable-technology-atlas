...
test('Definitive v2入力欠落をGap付きでRouteし完成へ昇格しない',()=>{const result=route(index,{subjectId:'zero-trust'});const definitive=result.candidates[0].definitiveV2;assert.equal(definitive.status,'subject-definitive-input-missing');assert.equal(definitive.completionClass,'not-definitive');assert.equal(definitive.coreGate.result,'not-run');assert.equal(definitive.autoPromotion,false);assert.ok(definitive.gapIds.includes('definitive.yaml'));});
