...
import { projectFixedCommitAudit, validateFixedCommitAudit } from './lib/fixed-commit-audit.mjs';
...
for (const domain of catalog.domains) for (const subject of domain.subjects) {
  ...
  const fixedCommitAuditIndex=fixedCommitAudit?{schemaVersion:fixedCommitAudit.schemaVersion,subjectId:fixedCommitAudit.subjectId,atlasId:fixedCommitAudit.atlasId,status:fixedCommitAudit.status,source:{repository:fixedCommitAudit.source.repository,commit:fixedCommitAudit.source.commit,tree:fixedCommitAudit.source.tree,attestationDigest:fixedCommitAudit.source.attestationDigest},manifest:{status:fixedCommitAudit.manifest.status,openRequired:fixedCommitAudit.manifest.openRequired},gapCount:fixedCommitAudit.gaps.length,detailUrl:`/data/fixed-commit-audits/${subject.id}/${fixedCommitAudit.source.attestationDigest.replace(/^sha256:/,'')}.json`,readOnly:true,autoPromotion:false}:null;
...
const index = {
  ...
  fixedCommitAuditSummary:{available:fixedCommitAudits.size,incomplete:subjects.filter((item)=>item.fixedCommitAudit?.status==='fixed-commit-incomplete').length,releaseEligible:0,gapInstances:[...fixedCommitAudits.values()].reduce((sum,item)=>sum+item.gaps.reduce((count,gap)=>count+gap.count,0),0),readOnly:true,autoPromotion:false},
