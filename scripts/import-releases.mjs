...
const index = {
  ...
  definitiveV2Summary:{coreCommit:definitiveV2Lock.coreCommit,contractStatus:'final',subjects:subjects.length,available:subjects.filter((item)=>item.definitiveV2.availability==='available').length,definitive:subjects.filter((item)=>item.definitiveV2.status==='subject-definitive').length,incomplete:subjects.filter((item)=>item.definitiveV2.status==='subject-definitive-incomplete').length,missing:subjects.filter((item)=>item.definitiveV2.status==='subject-definitive-input-missing').length,autoPromotion:false},
...
const verdict = subjects.length === 97 && imports.every((item) => item.verification === 'verified') && depthImports.length === 1 && depthImports.every((item) => item.verification === 'verified') && reviewImports.length===1 && reviewImports.every((item)=>item.verification==='verified')&&evidenceDependencyImports.every((item)=>item.verification==='verified')&&definitiveV2Imports.every((item)=>item.verification==='verified') ? 'pass' : 'fail';
await writeFile(reportTemporary, `${JSON.stringify({ schemaVersion: 1, catalog: catalogVerification, imports, depthImports, reviewImports, evidenceDependencyImports, definitiveV2Imports, index: { path: path.relative(root, output), digest: index.digest, subjects: subjects.length }, verdict }, null, 2)}\n`);
