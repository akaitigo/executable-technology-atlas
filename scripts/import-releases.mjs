...
const fixedCommitAuditDocuments=new Map([...fixedCommitAudits].map(([subjectId,value])=>{const document=`${JSON.stringify(value,null,2)}\n`;return[subjectId,{document,artifactDigest:sha256(Buffer.from(document))}];}));
const releaseDetailDocuments=new Map();for(const domain of catalog.domains)for(const subject of domain.subjects)for(const detail of releaseDetailsByRepository.get(subject.repository)??[]){const contentId=detail.digest.replace(/^sha256:/,'');const sourceDetail={...detail,...(depthReferences.has(subject.id)?{depthReference:depthReferences.get(subject.id)}:{}),detailUrl:`/data/releases/${subject.id}/${contentId}.json`};const projectedDetail=neutralizeDisplayText(sourceDetail);const transformations=JSON.stringify(projectedDetail)===JSON.stringify(sourceDetail)?[]:['definitive-marketing-to-neutral-fact'];const document=`${JSON.stringify({...projectedDetail,displayProjection:{sourceReleaseDigest:detail.digest,transformations}},null,2)}\n`;releaseDetailDocuments.set(`${subject.repository}:${detail.digest}`,{document,artifactDigest:sha256(Buffer.from(document))});}
...
for (const domain of catalog.domains) for (const subject of domain.subjects) {
  ...
  let releaseHistory = (releasesByRepository.get(subject.repository) ?? []).map((item) => ({ ...item, detailUrl: `/data/releases/${subject.id}/${item.digest.replace(/^sha256:/, '')}.json`, artifactDigest:releaseDetailDocuments.get(`${subject.repository}:${item.digest}`).artifactDigest }));
...
if (verdict === 'fail') {
  ...
  const fixedAuditRoot=path.join(root,'public','data','fixed-commit-audits');for(const[subjectId,value]of fixedCommitAudits){const subjectRoot=path.join(fixedAuditRoot,subjectId);await mkdir(subjectRoot,{recursive:true});await writeFile(path.join(subjectRoot,`${value.source.attestationDigest.replace(/^sha256:/,'')}.json`),fixedCommitAuditDocuments.get(subjectId).document);}
  ...
  for (const [repository, details] of releaseDetailsByRepository) {
    ...
    for (const detail of details) {
      ...
      await writeFile(path.join(subjectDetailRoot, `${contentId}.json`), releaseDetailDocuments.get(`${repository}:${detail.digest}`).document);
    }
  }
