...
const index = {
  ...
  fixedCommitAuditSummary:{available:fixedCommitAudits.size,incomplete:subjects.filter((item)=>item.fixedCommitAudit?.status==='fixed-commit-incomplete').length,releaseEligible:0,gapInstances:subjects.reduce((sum,item)=>sum+(item.fixedCommitAudit?.gaps.reduce((count,gap)=>count+gap.count,0)??0),0),readOnly:true,autoPromotion:false},
