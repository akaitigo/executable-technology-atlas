export function route(index, request) {
  ...
  return { mode, decision, candidates: candidates.map((item) => ({ id:item.id, catalogStatus:item.status, completion:item.completion, depthReference:item.depthReference, scope:item.scope, exclusions:item.excludes, release:item.release ? { version:item.release.version, status:item.release.status, completion:item.release.completion, uri:item.release.uri, digest:item.release.digest, verification:item.release.verification, coverage:item.release.coverage, evidenceCount:item.release.evidenceCount, requiredProfiles:item.release.requiredProfiles, observedProfiles:item.release.observedProfiles, skill:item.release.skill } : null })), indexDigest:index.digest };
}
