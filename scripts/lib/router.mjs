export function route(index, request) {
  ...
  return { mode, decision, candidates: candidates.map((item) => ({ id:item.id, catalogStatus:item.status, scope:item.scope, exclusions:item.excludes, release:item.release ? { version:item.release.version, status:item.release.status, uri:item.release.uri, digest:item.release.digest, verification:item.release.verification, coverage:item.release.coverage, evidence:item.release.evidence, skill:item.release.skill } : null })), indexDigest:index.digest };
}
