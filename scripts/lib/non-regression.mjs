...
export async function evaluateNonRegression(root = process.cwd(), options = {}) {
  ...
  check('index-bootstrap-fail-closed',bootstrap?.subjects===index.subjects.length&&bootstrap?.readOnly===true&&bootstrap?.autoPromotion===false&&bootstrap?.completionSummary?.subjectDefinitive===index.completionSummary.subjectDefinitive&&bootstrap?.definitiveV2Summary?.missing===index.definitiveV2Summary.missing&&bootstrap?.definitiveV2Summary?.gapInstances===index.definitiveV2Summary.gapInstances&&bootstrap?.definitiveV2Summary?.autoPromotion===false&&bootstrap?.evidenceDependencySummary?.missing===index.evidenceDependencySummary.missing&&bootstrap?.fixedCommitAuditSummary?.incomplete===index.fixedCommitAuditSummary.incomplete&&bootstrap?.fixedCommitAuditSummary?.releaseEligible===0&&bootstrap?.fallback?.status==='index-unavailable-not-evaluated','bootstrap must preserve incomplete summaries');
  ...
  for(const token of ['実Subject固定commit監査','固定Evidenceはあるが、署名済みReleaseではない','fixed-commit-incomplete','Release未成立','audit.gaps.map(','固定commit監査はRelease入力欠落を埋めません','bounded open required 0でもSubject Definitive完成ではありません','audit.depthReference.axes.map(','Subject Depth Reference: 18軸の状態・分母・Proof・Gap'])if(!page.includes(token))fail('fixed-commit-audit-ui-reduced',token);
