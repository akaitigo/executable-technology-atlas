...
export const POSTGRESQL_FIXED_COMMIT_AUDIT_LOCK_DIGEST = 'sha256:5a18f05c0136587822773cf3d2f835229fb4d8e6db6ef5e783a919747b5294e9';
...
export async function evaluateNonRegression(root = process.cwd(), options = {}) {
  ...
  const [baselineBytes, baseline, mappings, index, failures, page, depthLockBytes, depthLock, reviewLockBytes, reviewLock, dependencyLockBytes, dependencyLock, definitiveLockBytes, definitiveLock, fixedAuditLockBytes, fixedAuditLock, postgresqlAuditLockBytes, postgresqlAuditLock, registry] = await Promise.all([
  ...
  for(const [lock,openRequired] of [[fixedAuditLock,22],[postgresqlAuditLock,27]]){const fixedAuditSubject=currentSubjects.get(lock.subjectId);const fixedAudit=fixedAuditSubject?.fixedCommitAudit;if(!fixedAudit)fail('fixed-commit-audit-hidden',lock.subjectId);else{if(fixedAudit.source?.commit!==lock.sourceCommit||fixedAudit.source?.tree!==lock.sourceTree||fixedAudit.source?.attestationDigest!==lock.attestationDigest)fail('fixed-commit-audit-source-rewritten',lock.subjectId);if(fixedAudit.status!=='fixed-commit-incomplete'||fixedAudit.readOnly!==true||fixedAudit.autoPromotion!==false||fixedAudit.releaseBoundary?.status!=='unpublished-fixed-commit'||fixedAudit.releaseBoundary?.signedManifest!==false||fixedAudit.releaseBoundary?.publicTrustKey!==false||fixedAudit.releaseBoundary?.definitiveCertificate!==false)fail('fixed-commit-audit-promoted',lock.subjectId);if(fixedAudit.core?.definitive?.result!=='fail'||fixedAudit.manifest?.status!=='incomplete'||fixedAudit.manifest?.openRequired!==openRequired)fail('fixed-commit-audit-gap-status-rewritten',lock.subjectId);if(JSON.stringify(fixedAudit.gaps?.map((gap)=>gap.id))!==JSON.stringify(lock.requiredGapIds))fail('fixed-commit-audit-gaps-reduced',lock.subjectId);}}
