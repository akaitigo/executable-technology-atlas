...
export function projectFixedCommitAudit(envelope,validated){
  const value=envelope.payload;const summary=(gate,keys)=>({result:gate.result,summary:Object.fromEntries(keys.filter((key)=>gate.summary[key]!==undefined).map((key)=>[key,gate.summary[key]])),...(gate.diagnostics.length?{diagnostics:gate.diagnostics}:{})});const optional=(key,keys)=>value.core[key]?{[key]:summary(value.core[key],keys)}:{};return{schemaVersion:1,subjectId:value.subjectId,atlasId:value.atlasId,status:'fixed-commit-incomplete',source:{repository:value.repository,commit:value.sourceCommit,tree:value.sourceTree,mode:value.sourceMode,attestationDigest:validated.digest,trust:validated.trust},releaseBoundary:value.releaseBoundary,manifest:value.manifest,core:{commit:value.core.commit,audit:summary(value.core.audit,['completionClass','targets','claims','evidence','openRequired']),evidenceDependency:summary(value.core.evidenceDependency,['inputs','changedInputs','outputs','affectedOutputs','runs','missingRequiredOutputs']),authorityExtraction:summary(value.core.authorityExtraction,['status','locked','matched','failed','staleSources','deferredLocators','humanReviewed','coreV2Eligible']),authorityBody:summary(value.core.authorityBody,['status','sources','documents','matched','failed','candidateAnchors','classified','unclassified','humanReviewed','coreV2Eligible']),authorityReview:summary(value.core.authorityReview,['status','queued','pendingHuman','humanReviewed','unavailableHolds','decisions','packets','candidateProjections','machineProposals','staleHolds']),definitive:summary(value.core.definitive,['completionClass','declaredCompletionClass','missingContractArtifacts','openRequired']),...optional('verificationMatrix',['rows','closedRows','gapRows','variantCells','closedVariantCells','completionEligibleRows']),...optional('scenarioTrace',['status','rows','runtimeRows','variantCells','closedVariantCells','gaps','authorityAtomicRows','completionEligibleRows']),...optional('nonRegression',['missingContractArtifacts','baselineItems','currentItems','replacements']),...optional('evidenceDurability',['missingRequiredOutputs','status','artifacts','rows','variants','total','passed','failed'])},...(value.depthReference?{depthReference:value.depthReference}:{}),gaps:value.gaps,readOnly:true,autoPromotion:false};
}
