...
export function missingDefinitiveV2(subject,release,lock){
  const states=release?.coverage?.states??{};
  return{schemaVersion:1,subjectId:subject.id,atlasId:release?.atlasId??null,availability:'missing',status:lock.missingInputState,completionClass:'not-definitive',coreContract:{status:'final',commit:lock.coreCommit},coreGate:{result:'not-run',command:lock.gateCommand,diagnostics:['署名済みDefinitive v2 bundleとCore Gate結果がありません']},migration:{status:'not-started',requiredActions:[],historicalCertificate:null},inventoryClosure:{status:'not-evaluated',required:null,classified:null,unclassified:null,openRequired:release?.coverage?.openRequired??null,excluded:states.excluded??0,infeasible:states.infeasible??0},runtimeProfiles:(release?.observedProfiles??[]).map((profile)=>({profile,status:'v1-evidence-observed-not-v2-verified',runtimeIdentity:null})),gapIds:[...lock.missingRequiredInputs],artifacts:null,certificate:null,source:null,readOnly:true,autoPromotion:false,detailUrl:null};
}
