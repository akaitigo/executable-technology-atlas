...
if (command === 'import') run('import-releases.mjs', args);
  ...
  console.log(JSON.stringify(matches.map((subject) => ({ id: subject.id, title: subject.title, domain: subject.domain.title, catalogStatus: subject.status, release: subject.release ? { version: subject.release.version, status: subject.release.status, verification: subject.release.verification, digest: subject.release.digest } : null })), null, 2));
} else {
