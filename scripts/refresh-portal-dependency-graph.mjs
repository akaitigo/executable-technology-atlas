...
import { dependencyReproducePlan, loadPortalDependencyReproduceOrder, validatePortalDependencyReproduceOrder } from './lib/portal-dependency-reproduce-order.mjs';
...
const {order,schema:orderSchema,packageDocument}=await loadPortalDependencyReproduceOrder(root);const orderValidation=validatePortalDependencyReproduceOrder(order,orderSchema,packageDocument);if(!orderValidation.ok)throw new Error(orderValidation.errors.join('; '));
...
if(mode==='--check'){
  ...
  const startedAt=new Date().toISOString();const inputs=await resolveInputGroups(root,config,previous,startedAt);
  const plan=dependencyReproducePlan(order);
  for(const script of plan){const result=spawnSync('npm',['run',script],{cwd:root,stdio:'inherit',env:{...process.env,PORTAL_DEPENDENCY_REFRESH:'1'}});if(result.status!==0)throw new Error(`npm run ${script} が失敗しました`);}
