  ...
  return (
    <>
      ...
      <main id="top">
        ...
        <DefinitiveV2Workspace subjects={atlasIndex.subjects.map((item)=>({id:item.id,title:item.title,definitive:item.definitiveV2 as DefinitiveV2Index}))} />
...
function SubjectDetail({ subject, close }: { subject:Subject; close:()=>void }) {
  ...
  return <div className="detail"><header><div><p className="eyebrow">{subject.domain.title} / {subject.id}</p><h2 id="detail-title">{subject.title}</h2></div><button className="close" type="button" onClick={close} aria-label="詳細を閉じる">×</button></header><section><h3>Catalog境界</h3><p>{subject.scope}</p><ul>{subject.excludes.map((item) => <li key={item}>{item}</li>)}</ul></section><DefinitiveV2View definitive={subject.definitiveV2 as DefinitiveV2Index} />
...
function DefinitiveV2Workspace({ subjects }: { subjects:{id:string;title:string;definitive:DefinitiveV2Index}[] }) {
  ...
  const inventory=definitive.inventoryClosure;
  return <section className="definitive-workspace" id="definitive-v2" aria-labelledby="definitive-v2-title"><div className="definitive-heading"><div><p className="eyebrow">Core Definitive Gate v2 / fail closed</p><h2 id="definitive-v2-title">履歴上のClosureと、現在の完成を混同しない。</h2></div><p>Core正式mainの確定Schema・Migration・Gate結果を固定Digestで検証するread-only表示です。署名済みSubject bundle、公開Trust、Gate passが揃わないSubjectは未完成のままです。</p></div><div className="definitive-summary" aria-label="Definitive v2集計"><div><strong>{subjects.length}</strong><span>Subject</span></div><div><strong>{definitiveCount}</strong><span>subject-definitive</span></div><div><strong>{incomplete}</strong><span>Gate未通過</span></div><div><strong>{missing}</strong><span>固定入力なし</span></div></div><p className="definitive-warning"><strong>非昇格:</strong> bounded-complete / bounded historical、Manifest <code>complete</code>、Test成功件数だけではSubject Definitiveではありません。既知Gap、excluded、infeasible、unclassified、open requiredを隠しません。</p><label className="dependency-select">Subject<select value={subjectId} onChange={(event)=>setSubjectId(event.target.value)}>{subjects.map((item)=><option key={item.id} value={item.id}>{item.title} · {item.definitive.status}</option>)}</select></label><div className="definitive-gate"><span className={`status ${definitive.status==='subject-definitive'?'release-definitive':'release-incomplete'}`}>{definitive.status}</span><dl><div><dt>Core正式main</dt><dd><code>{definitive.coreContract.commit}</code></dd></div><div><dt>Gate command</dt><dd><code>{definitive.coreGate.command}</code></dd></div><div><dt>Gate result</dt><dd className={definitive.coreGate.result==='pass'?'pass':'fail'}>{definitive.coreGate.result}</dd></div><div><dt>Certificate</dt><dd>{definitive.certificate?<><code>schema {definitive.certificate.schemaVersion}</code><span>{definitive.certificate.completionClass} / {definitive.certificate.trust.usage}</span></>:'署名済みv2 Certificateなし'}</dd></div></dl></div><div className="definitive-grid"><section><h3>Authority-derived inventory closure</h3><dl><div><dt>Status</dt><dd>{inventory.status}</dd></div><div><dt>Required / classified</dt><dd>{inventory.required??'未評価'} / {inventory.classified??'未評価'}</dd></div><div><dt>Unclassified</dt><dd>{inventory.unclassified??'未評価'}</dd></div><div><dt>Open required</dt><dd>{inventory.openRequired??'未評価'}</dd></div><div><dt>Excluded / infeasible</dt><dd>{inventory.excluded} / {inventory.infeasible}</dd></div></dl></section><section><h3>実Runtime Profile</h3>{definitive.runtimeProfiles.length?<ul>{definitive.runtimeProfiles.map((profile,index)=><li key={`${profile.id??profile.profile??'profile'}-${index}`}><code>{profile.profile??profile.id??'unknown'}</code><span>{profile.status}</span><small>identity <code>{profile.runtimeIdentity?JSON.stringify(profile.runtimeIdentity):'v2未検証'}</code></small></li>)}</ul>:<p>v2 verified Runtime Profileなし。Evidence観測Profileをv2実Runtime identityへ読み替えません。</p>}</section></div><div className="definitive-grid"><section><h3>Migration actions</h3>{definitive.migration.requiredActions.length?<ul>{definitive.migration.requiredActions.map((action)=><li key={action}><code>{action}</code></li>)}</ul>:<p>Subject v2 migration入力なし。</p>}</section><section><h3>既知Gap</h3>{definitive.gapIds.length?<ul>{definitive.gapIds.map((gap)=><li key={gap}><code>{gap}</code></li>)}</ul>:<p>Core Gate検証上のGapなし。</p>}</section></div><p className="definitive-contract"><code>readOnly={String(definitive.readOnly)}</code> / <code>autoPromotion={String(definitive.autoPromotion)}</code>。Portalは書込みも自動昇格も行いません。digest変更だけを復旧済みとは表示せず、Core Gate結果を正本にします。</p></section>;
}
