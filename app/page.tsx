...
type AuthorityReviewExport = Omit<AuthorityReviewIndex,'exportUrl'> & { packets:{id:string;anchor_id:string;deep_link:string;candidate_edges:number;proposed_cluster_ids:string[];url:string}[];proposedClusters:{id:string;proposal_type:string;basis:string;basis_value:string;packet_ids:string[];candidate_edge_ids:string[];semantic_decision:'none-machine-proposal-only';human_reviewed:false}[];staleHolds:{document_id:string;document_url:string;locator:string;status:string;reason:string;locked_source_digest:string;fetched_digest:string}[];staleCandidateReport:{status:string;locked_digests_updated:0;human_choices:0};decisionBoundary:{export_accepts_writes:false;futureWriteBoundary:string;requiredHumanFields:string[]} };
  ...
  return (
    <>
      ...
      <header className="site-header">
        ...
        <nav aria-label="主要ナビゲーション"><a href="#explorer">探索</a><a href="#authority-review">Human Review</a><a href="#verification">検証</a><a href="#portal-certificate">Portal証明</a><a href="#contract">契約</a></nav>
...
function AuthorityReviewWorkspace({ subjects }: { subjects:{id:string;title:string;review:AuthorityReviewIndex}[] }) {
  ...
  const fullReview=reviewExport?.subjectId===subjectId?reviewExport:null;
  ...
  useEffect(()=>{if(!effectivePacketId||!fullReview)return;let active=true;const record=fullReview.packets.find((item)=>item.id===effectivePacketId);if(!record)return;fetch(record.url).then((response)=>{if(!response.ok)throw new Error('packet unavailable');return response.json();}).then((value)=>{if(active){setPacket(value);setLoadError(false);}}).catch(()=>{if(active){setPacket(null);setLoadError(true);}});return()=>{active=false}},[effectivePacketId,fullReview]);
  ...
  const proposals=packet&&fullReview?fullReview.proposedClusters.filter((item)=>packet.proposed_cluster_ids.includes(item.id)):[];
